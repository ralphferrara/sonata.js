        ...
        export default class TestUpload {
                ...
                static async init(chirp: Chirp, parsed: ComponentData) : Promise<ComponentData> {
                        ...
                        if (isNaN(+parsed.attributes["data-fidarea"])) {
                              parsed.html = "<h2>Error : TestUpload is Missing FIDAREA</h2>" + parsed.html;
                              return parsed;
                        }
                        ...
                        //|| Add the Upload JWT
                        //||=-=-=-=-=-=-=-=-=--=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-||*/
                        let jwt           = await Media.uploadJWT(chirp.user.idUser, parsed.attributes["data-area"], parseInt(parsed.attributes["data-fidarea"]));
                        console.log(jwt);
